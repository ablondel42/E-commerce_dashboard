import { formatDataWithConfig } from "../../utils/dataTransform/dataFormatting";
...
const ExpensePrediction = ({ gridArea = "i", startDate, endDate }) => {
  ...
  const formattedData = useMemo(() => {
    if (!isLoading) {
      ...
      const line = formattedData.map((data, index) => {
      ...
      const future =
        ...
        1 -
        startDate.get("month");
      const finalData = formattedData.map(({ name, expenses }, index) => {
        return {
